...
class Worker(WorkerBase):
    def __init__(
        ...
        self._b12x_post_warmup_cache_released = False
    ...
    def compile_or_warm_up_model(self) -> CompilationTimes:
        ...
        if (
            _B12X_EMPTY_CACHE_AFTER_WARMUP
            and not self._b12x_post_warmup_cache_released
        ):
            ...
            mib = 1 << 20
            logger.info(
                "Post-warmup CUDA allocator cache release on %s: "
                "%.2f MiB reserved before, %.2f MiB reserved after "
                "(remaining), %.2f MiB released.",
                self.device,
                reserved_before / mib,
                reserved_after / mib,
