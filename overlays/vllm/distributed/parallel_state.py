...
class GroupCoordinator:
    ...
    def reduce_scatter_into(
        ...
        """Direct eager DCP reduce-scatter into caller-provided CUDA storage."""
        if self.world_size != 4:
            raise RuntimeError(
                "GroupCoordinator.reduce_scatter_into requires the validated "
                f"world size 4, got {self.world_size}."
            )
        if dim != 1:
            raise ValueError(
                "GroupCoordinator.reduce_scatter_into is restricted to DCP "
                f"head dimension 1, got dim={dim}."
            )
        if input_.dim() != 3 or output.dim() != 3 or input_.shape[0] == 0:
            raise ValueError(
                "GroupCoordinator.reduce_scatter_into requires nonempty rank-3 "
                f"input/output, got {tuple(input_.shape)} and {tuple(output.shape)}."
            )
        if (
            input_.device.type != "cuda"
            or output.device != input_.device
            or input_.device.index is None
        ):
            raise ValueError(
                "GroupCoordinator.reduce_scatter_into requires input and output "
                f"on one explicit CUDA device, got {input_.device} and "
                f"{output.device}."
            )
        current_device = torch.cuda.current_device()
        if current_device != input_.device.index:
            raise RuntimeError(
                "GroupCoordinator.reduce_scatter_into requires the input device "
                f"to be current, got current cuda:{current_device}, input "
                f"{input_.device}."
            )
        if torch.cuda.is_current_stream_capturing():
            raise RuntimeError(
                "GroupCoordinator.reduce_scatter_into is a direct eager-only API "
                "and cannot run during CUDA graph capture."
            )
        if self.device_communicator is None:
            raise RuntimeError(
                "GroupCoordinator.reduce_scatter_into requires a device "
                "communicator."
            )
        if (
            getattr(self.device_communicator, "world_size", None) != self.world_size
            or getattr(self.device_communicator, "rank_in_group", None)
            != self.rank_in_group
        ):
            raise RuntimeError(
                "GroupCoordinator.reduce_scatter_into device communicator group "
                "does not match the coordinator."
            )
        ...
        if not callable(reduce_scatter_into):
            raise RuntimeError(
                ...
                "direct reduce_scatter_into."
            )
        result = reduce_scatter_into(input_, output, dim)
        if result is not output:
            raise RuntimeError(
                "Device communicator reduce_scatter_into did not return the "
