...
class CudaCommunicator(DeviceCommunicatorBase):
    ...
    def reduce_scatter_into(
        ...
        """Run the validated eager DCP reduce-scatter into borrowed storage."""
        world_size = self.world_size
        if world_size != 4:
            raise RuntimeError(
                "CudaCommunicator.reduce_scatter_into requires the validated "
                f"world size 4, got {world_size}."
            )
        if self.rank != self.rank_in_group or not 0 <= self.rank_in_group < world_size:
            raise RuntimeError(
                "CudaCommunicator.reduce_scatter_into has inconsistent group "
                f"rank state: rank={self.rank}, rank_in_group={self.rank_in_group}, "
        ...
        if (
            ...
            or not getattr(pynccl_comm, "available", False)
        ):
            raise RuntimeError(
                "CudaCommunicator.reduce_scatter_into requires an enabled "
                "available PyNccl communicator."
            )
        if (
            pynccl_comm.world_size != world_size
            or pynccl_comm.rank != self.rank_in_group
            or pynccl_comm.device != self.device
        ):
            raise RuntimeError(
                "CudaCommunicator.reduce_scatter_into PyNccl group mismatch: "
                f"coordinator rank/world/device={self.rank_in_group}/"
                f"{world_size}/{self.device}, PyNccl rank/world/device="
                f"{pynccl_comm.rank}/{pynccl_comm.world_size}/"
                f"{pynccl_comm.device}."
            )
        if dim != 1:
            raise ValueError(
                "CudaCommunicator.reduce_scatter_into is restricted to DCP "
                f"head dimension 1, got dim={dim}."
            )
        if input_.dim() != 3 or output.dim() != 3:
            raise ValueError(
                "CudaCommunicator.reduce_scatter_into requires rank-3 input and "
                f"output, got rank {input_.dim()} and {output.dim()}."
            )
        num_tokens = int(input_.shape[0])
        if (
            num_tokens < 1025
            or num_tokens > 3072
            or tuple(input_.shape) != (num_tokens, 64, 256)
            or tuple(input_.stride()) != (256, num_tokens * 256, 1)
            or tuple(output.shape) != (num_tokens, 16, 256)
            or tuple(output.stride()) != (256, num_tokens * 256, 1)
        ):
            raise ValueError(
                "CudaCommunicator.reduce_scatter_into requires exact token-major "
                "DCP layouts input=[T,64,256], output=[T,16,256], "
                "stride=(256,T*256,1), T=1025..3072; got "
                f"input shape/stride={tuple(input_.shape)}/{tuple(input_.stride())}, "
                f"output shape/stride={tuple(output.shape)}/{tuple(output.stride())}."
            )
        if (
            ...
            raise ValueError(
                "CudaCommunicator.reduce_scatter_into requires CUDA BF16 input "
                f"and output on exact communicator device {self.device}, got "
                f"input={input_.dtype}/{input_.device}, "
                f"output={output.dtype}/{output.device}."
            )
        current_device = torch.cuda.current_device()
        if current_device != self.device.index:
            raise RuntimeError(
                "CudaCommunicator.reduce_scatter_into requires the communicator "
                f"device to be current, got current cuda:{current_device}, "
                f"communicator {self.device}."
            )
        if torch.cuda.is_current_stream_capturing():
            raise RuntimeError(
                "CudaCommunicator.reduce_scatter_into is eager-only and cannot "
                "run during CUDA graph capture."
            )

        input_tensor = input_.movedim(0, dim)
        output_tensor = output.movedim(0, dim)
        if (
            tuple(input_tensor.shape) != (64, num_tokens, 256)
            or tuple(input_tensor.stride()) != (num_tokens * 256, 256, 1)
            or not input_tensor.is_contiguous()
        ):
            raise ValueError(
                "CudaCommunicator.reduce_scatter_into requires "
                "input_.movedim(0, 1) to already be the exact contiguous "
                f"[64,T,256] input, got shape={tuple(input_tensor.shape)}, "
                f"stride={tuple(input_tensor.stride())}."
            )
        if (
            tuple(output_tensor.shape) != (16, num_tokens, 256)
            or tuple(output_tensor.stride()) != (num_tokens * 256, 256, 1)
            or not output_tensor.is_contiguous()
        ):
            raise ValueError(
                "CudaCommunicator.reduce_scatter_into requires "
                "output.movedim(0, 1) to already be the exact contiguous "
                f"[16,T,256] output, got shape={tuple(output_tensor.shape)}, "
                f"stride={tuple(output_tensor.stride())}."
            )
        if input_tensor.numel() != world_size * output_tensor.numel():
            raise ValueError(
                "CudaCommunicator.reduce_scatter_into element-count mismatch: "
        ...
        if input_begin < output_end and output_begin < input_end:
            raise ValueError(
                "CudaCommunicator.reduce_scatter_into requires fully disjoint "
